...
SubGhzProtocolStatus tpms_block_generic_serialize(
    ...
    do {
        ...
        /* Optional lossless payload extension. Old readers ignore extra keys. */
        if(instance->raw_payload_valid && instance->raw_payload_size > 0U &&
           instance->raw_payload_size <= sizeof(instance->raw_payload)) {
            temp_data = instance->raw_payload_size;
            if(!flipper_format_write_uint32(flipper_format, "RawPayloadSize", &temp_data, 1)) {
                res = SubGhzProtocolStatusErrorParserOthers;
                break;
            }
            temp_data = instance->raw_payload_bits;
            if(!flipper_format_write_uint32(flipper_format, "RawPayloadBits", &temp_data, 1)) {
                res = SubGhzProtocolStatusErrorParserOthers;
                break;
            }
            if(!flipper_format_write_hex(
...
SubGhzProtocolStatus
    ...
    do {
        ...
        instance->raw_payload_size = 0U;
        instance->raw_payload_bits = 0U;
        instance->raw_payload_valid = false;
        if(flipper_format_rewind(flipper_format)) {
            uint32_t raw_size = 0U;
            if(flipper_format_read_uint32(flipper_format, "RawPayloadSize", &raw_size, 1) &&
               raw_size > 0U && raw_size <= sizeof(instance->raw_payload)) {
                uint32_t raw_bits = raw_size * 8U;
                if(flipper_format_rewind(flipper_format)) {
                    (void)flipper_format_read_uint32(
                        flipper_format, "RawPayloadBits", &raw_bits, 1);
                }
                if(flipper_format_rewind(flipper_format) &&
                   flipper_format_read_hex(
                       flipper_format, "RawPayload", instance->raw_payload, raw_size)) {
                    instance->raw_payload_size = (uint8_t)raw_size;
                    instance->raw_payload_bits =
                        (uint8_t)(raw_bits > 128U ? 128U : raw_bits);
                    instance->raw_payload_valid = true;
                }
            }
        }
