...
extern ProtoViewDecoder BMWGen23RX19TPMSDecoder;
extern ProtoViewDecoder Elantra2012RX19TPMSDecoder;
extern ProtoViewDecoder HyundaiVDORX19TPMSDecoder;
extern ProtoViewDecoder TruckRX19TPMSDecoder;
extern ProtoViewDecoder Renault0435RRX19TPMSDecoder;
extern ProtoViewDecoder HondaTRWRX19TPMSDecoder;
extern ProtoViewDecoder PorscheRX19TPMSDecoder;
extern ProtoViewDecoder KiaRX19TPMSDecoder;
extern ProtoViewDecoder PMV107JRX18TPMSDecoder;
extern ProtoViewDecoder TG1CRX18TPMSDecoder;
extern ProtoViewDecoder Q85RX18TPMSDecoder;
extern ProtoViewDecoder MercedesSprinterRX18TPMSDecoder;
...
ProtoViewDecoder *Decoders[] = {
    ...
    /* EXP: checked only after all six proven ProtoView decoders fail. */
    &BMWGen23RX19TPMSDecoder,
    &Elantra2012RX19TPMSDecoder,
    &HyundaiVDORX19TPMSDecoder,
    &TruckRX19TPMSDecoder,
    &Renault0435RRX19TPMSDecoder,
    &HondaTRWRX19TPMSDecoder,
    &PorscheRX19TPMSDecoder,
    &KiaRX19TPMSDecoder,

    /* TEST verified source layouts. Q85 precedes TG1C because both share the
     * same rtl_433 family/preamble and Q85 extends TG1C with CRC16. */
    &PMV107JRX18TPMSDecoder,
    &Q85RX18TPMSDecoder,
    &TG1CRX18TPMSDecoder,
    &MercedesSprinterRX18TPMSDecoder,
    NULL
};

uint8_t tpms_decoder_modulation(const ProtoViewDecoder* decoder) {
    if(decoder == &SchraderTPMSDecoder || decoder == &SchraderEG53MA4TPMSDecoder) {
        return ProtoTpmsModulationOOK;
    }
    return ProtoTpmsModulationFSK;
}

bool tpms_decoder_matches_modulation(const ProtoViewDecoder* decoder, uint8_t modulation) {
    const bool decoder_ook = tpms_decoder_modulation(decoder) == ProtoTpmsModulationOOK;
    const bool selected_ook = modulation == ProtoTpmsModulationOOK;
    const bool selected_fsk = modulation == ProtoTpmsModulationFSK;
    return (decoder_ook && selected_ook) || (!decoder_ook && selected_fsk);
}
...
uint32_t search_coherent_signal(RawSamplesBuffer *s, uint32_t idx, uint32_t min_duration) {
    ...
    for (uint32_t j = idx; j < idx+s->total; j++) {
        bool level;
        uint32_t dur;
        raw_samples_get_unlocked(s, j, &level, &dur);
    ...
    while (i < copy->total-1) {
        ...
        if (thislen > minlen) {
                 ...
                 decoded))
            {
                ...
                /* The classic TPMS UI owns notifications after storing a
                 * decoded sensor. Avoid a second vibration here. */
            } else {
...
uint32_t convert_signal_to_bits(uint8_t *b, uint32_t blen, RawSamplesBuffer *s, uint32_t idx, uint32_t count, uint32_t rate) {
    ...
    for (uint32_t j = 0; j < count; j++) {
        uint32_t dur;
        bool level;
        raw_samples_get_unlocked(s, j+idx, &level, &dur);
...
void init_msg_info(ProtoViewMsgInfo *i, ProtoViewApp *app) {
    ...
    i->bits = NULL;
    i->modulation = app ? app->modulation : ProtoTpmsModulationFSK;
...
bool decode_signal(RawSamplesBuffer *s, uint64_t len, ProtoViewMsgInfo *info) {
    ...
    uint8_t *bitmap = calloc(1U, bitmap_size);
    if(!bitmap) return false;
    uint32_t bits = convert_signal_to_bits(
        bitmap,
        bitmap_size,
        s,
        (uint32_t)(0U - before_samples),
        (uint32_t)len + before_samples + after_samples,
        s->short_pulse_dur);
    ...
    bool decoded = false;
    while(Decoders[j]) {
        if(!tpms_decoder_matches_modulation(Decoders[j], info->modulation)) {
            j++;
            continue;
        }
#if DEBUG_MSG
...
ProtoViewField* proto6_field_find(ProtoViewFieldSet* fs, const char* name) {
    ...
    for(uint32_t i = 0; i < fs->numfields; i++) {
        if(fs->fields[i] && fs->fields[i]->name && strcmp(fs->fields[i]->name, name) == 0) {
            return fs->fields[i];
        }
    }
    return NULL;
