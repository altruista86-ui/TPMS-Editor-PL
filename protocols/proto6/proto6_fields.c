...
static char* proto6_strdup(const char* source) {
...
int field_to_string(char *buf, size_t len, ProtoViewField *f) {
    ...
    case FieldTypeHex:
        return snprintf(buf, len, "%*llX", (int)(f->len+7)/8, (unsigned long long)f->uvalue);
    case FieldTypeFloat:
